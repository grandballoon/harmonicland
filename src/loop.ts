/* ====================================================================
   ...
   2. EDGES SIT ON BARLINES. The loop is a run of whole bars — the same
      BarRange practice mode isolates, and the same two bar-number boxes
      set it — so it is chosen in the unit a musician counts in, and an
      edge can never cut a note at the wrap or start the loop in silence
      halfway through a beat.

   3. MARKING IS INCLUSIVE. `[` and `]` are pressed while standing in a
      bar, and both mean "this bar is in".
   ...
   ==================================================================== */
import type { Bar, BarRange, TimeRange } from "./types";
...
/** Move one edge of a bar selection (or of the whole piece, when there is
 *  none) to `bar`. An edge marked past the other one does not swap them or
 *  refuse: the other edge gives way to its end of the piece, because the
 *  edge being set is the one the user is looking at. */
export function withBar(
  range: BarRange | null, edge: Edge, bar: number, bars: number,
): BarRange {
  const cur = range ?? { from: 0, to: bars - 1 };
  if (edge === "start") return { from: bar, to: bar <= cur.to ? cur.to : bars - 1 };
  return { from: bar >= cur.from ? cur.from : 0, to: bar };
}
...
export const Loop = { landing, stepFrom, withBar, snapBar };
